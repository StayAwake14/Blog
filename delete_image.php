...
<head>
	...
	<script>
		function deleteImage(){
			...
			swal({
				  title: "Potwierdzenie",
				  text: "Czy na pewno chcesz usunąć obrazek?",
				  type: "info",
				...
				function(){
				  setTimeout(function(){
					swal("Obrazek został skasowany z galerii!");
				  }, 2000);
	...
	<div id="main">
		...
		<div id="center">
			...
			{	
					...
					{
					echo <<<END
					<h1 style="padding: 20px 0px;">Wybierz obrazek</h1>
					<form id="form" action="delete_image.php" method="POST" onSubmit="return deleteImage()">
...
END;
					...
					<button onClick="return deleteImage()" type="button">Usuń</button>
					</form>
...
END;
					...
					if(isset($_POST['post_name']))
					{
						...
						$sql_for_id = "SELECT * FROM gallery WHERE title='$title'";
						...
						if($exec = $connect -> query($sql_for_id))
						{
							...
							unlink('./img/gallery/gallery'.$id.'.jpg');
							$delete = "DELETE FROM gallery WHERE title='$title'";
							$connect -> query($delete);
							
														
							header("location: ./delete_image.php");
							
							echo "<br/>Usuwanie obrazka powiodło się!";
